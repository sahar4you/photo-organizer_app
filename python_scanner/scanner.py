...
def _process_face_batch(args):
    ...
    try:
        ...
        profile_cascade = cv2.CascadeClassifier(profile_path) if profile_path else None
        for filepath in filepaths:
            try:
                ...
                results.append((filepath, rects, embeddings, face_thumbs))
            except Exception:
...
def scan_folder(folder):
    ...
    for i, (filepath, rel_path) in enumerate(all_media):
        ...
        # Face detection deferred to parallel batch below
        face_count = 0
